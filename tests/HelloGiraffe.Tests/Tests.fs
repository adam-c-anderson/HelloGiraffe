...
let createHost() =
    WebHostBuilder()
        ...
        .Configure(Action<IApplicationBuilder> HelloGiraffe.App.configureApp)
        .ConfigureServices(Action<IServiceCollection> HelloGiraffe.App.configureServices)
...
let ``Route /api/hello returns "Hello world, from Giraffe!"`` () =
    ...
    |> ensureSuccess
    |> readText
    |> shouldContain "Hello world, from HelloGiraffe!"
